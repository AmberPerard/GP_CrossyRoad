...
void ShadowMapRenderer::UpdateMeshFilter(const SceneContext& sceneContext, MeshFilter* pMeshFilter) const
{
	...
	MaterialTechniqueContext materialTechnique = m_GeneratorTechniqueContexts[id];
	...
	pMeshFilter->BuildVertexBuffer(sceneContext, materialTechnique.inputLayoutID, materialTechnique.inputLayoutSize, materialTechnique.pInputLayoutDescriptions);
}
...
void ShadowMapRenderer::Begin(const SceneContext& sceneContext)
{
	TODO_W8(L"Implement Begin")
		...
		//1. Making sure that the ShadowMap is unbound from the pipeline as ShaderResourceView (SRV) is important, because we cannot use the same resource as a ShaderResourceView (texture resource inside a shader) and a RenderTargetView (target everything is rendered too) at the same time.
		//In case this happens, you'll see an error in the output of visual studio - warning you that a resource is still bound as a SRV and cannot be used as an RTV.
		//	-> Unbinding an SRV can be achieved using DeviceContext::PSSetShaderResource [I'll give you the implementation for free]
		//	- double check your output because depending on your usage of ShaderResources,
		//	the actual slot the ShadowMap is using can be different, but you'll see a warning pop-up with the correct slot ID in that case.
	...
	XMMATRIX projection = XMMatrixOrthographicLH(100.f * sceneContext.aspectRatio, 100.f, 0.1f, 500.f);
	...
	XMMATRIX view = XMMatrixLookAtLH(lightPos, lightPos + lightDir, { 0,1,0,0 });
...
void ShadowMapRenderer::DrawMesh(const SceneContext& sceneContext, MeshFilter* pMeshFilter, const XMFLOAT4X4& meshWorld, const std::vector<XMFLOAT4X4>& meshBones)
{
	...
	{
		pDeviceContext->IASetInputLayout(materialTechnique.pInputLayout);
		const UINT offset = 0;
		const auto& vertexBufferData = pMeshFilter->GetVertexBufferData(materialTechnique.inputLayoutID, subMesh.id);
		...
		materialTechnique.pTechnique->GetDesc(&techDesc);
		for (UINT p = 0; p < techDesc.Passes; ++p)
		{
			materialTechnique.pTechnique->GetPassByIndex(p)->Apply(0, pDeviceContext);
